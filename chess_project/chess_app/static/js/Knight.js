class Knight extends Piece {
    constructor(config) {
        ...
        this.on('dragend', function(pointer, dragX, dragY) {
            ...
            if((((Math.round(pointer.x/off_set)*off_set - 200) == this.prevXcoord) || ((Math.round(pointer.x/off_set)*off_set + 200) == this.prevXcoord)) && (((Math.round(pointer.y/off_set)*off_set - 100) == this.prevYcoord) || ((Math.round(pointer.y/off_set)*off_set + 100) == this.prevYcoord))) {
            ...
            else if((((Math.round(pointer.y/off_set)*off_set - 200) == this.prevYcoord) || ((Math.round(pointer.y/off_set)*off_set + 200) == this.prevYcoord)) && (((Math.round(pointer.x/off_set)*off_set - 100) == this.prevXcoord) || ((Math.round(pointer.x/off_set)*off_set + 100) == this.prevXcoord))) {
                this.x = Math.round(pointer.x/off_set)*off_set % 100 == 0 ? Math.round(pointer.x/off_set)*off_set + off_set : Math.round(pointer.x/off_set)*off_set;
                this.y = Math.round(pointer.y/off_set)*off_set % 100 == 0 ? Math.round(pointer.y/off_set)*off_set + off_set : Math.round(pointer.y/off_set)*off_set;
            }
            else {
